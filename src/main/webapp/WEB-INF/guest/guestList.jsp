...
<jsp:include page="/include/header.jsp" />
	<div class="container">
		...
		<div class="row">
			<div class="col">
				<c:if test="${sAdmin != 'adminOk'}">
					<a href="${ctp}/AdminLogin.gu" class="btn btn-success btn-sm">관리자모드</a>
				</c:if>
				<c:if test="${sAdmin == 'adminOk'}">
					<a href="${ctp}/AdminLogout.gu" class="btn btn-danger btn-sm">로그아웃</a>
				</c:if>
					<a href="${ctp}/GuestInput.gu" class="btn btn-primary btn-sm">등록</a>
			</div>
		...
		<div id="custom-responsive-table" class="mt-3">
			<table class="table">
				<colgroup>
					<col width="15%" />
					<col width="18%" />
				...
				<tr>
					...
					<td colspan="5">
						<div class="row">
							<div class="col-sm-7" id="noCnt">
								${curScrStartNo}
							</div>
							<c:if test="${sAdmin == 'adminOk'}">
								<div class="text-right col-sm-5" id="noCnt">
									<a href="javascript:delCheck(${vo.idx})" class="btn btn-dark btn-sm ml-2">삭제</a>
								</div>
							</c:if>
						</div>
					</td>
				...
				<tr>
					...
					<td style="">${vo.name}</td>
					<th style="">방문일</th>
					<td style="">${fn:substring(vo.visitDate,0,19)}</td>
					<th style="">IP</th>
					<td style="">${vo.hostIp}</td>
				</tr>
				<tr>
					<th>E.mail</th>
					<td colspan="5">
				...
				<tr>
					<th>방문소감</th>
					<td colspan="5" style="height: 150px">${fn:replace(vo.content, newLine, '<br/>')}</td>
				</tr>
			</table>
		</div>
		...
		<div class="text-center mt-2">
			<ul class="pagination justify-content-center">
				<c:if test="${pag > 1}">
					<li class="page-item">
						<a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=1">첫페이지</a>
					</li>
				</c:if>
				<c:if test="${curBlock > 0}">
					<li class="page-item">
						<a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=${(curBlock-1)*blockSize + 1}">이전블록</a>
					</li>
				</c:if>
				<c:forEach var="i" begin="${curBlock*blockSize + 1}" end="${curBlock*blockSize + blockSize}" varStatus="st">
					<c:if test="${i <= totPage && i == pag}">
						<li class="page-item active">
							<a class="page-link text-white bg-secondary border-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=${i}">${i}</a>
						</li>
					</c:if>
					<c:if test="${i <= totPage && i != pag}">
						<li class="page-item">
							<a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=${i}">${i}</a>
						</li>
					</c:if>
				</c:forEach>
				<c:if test="${curBlock < lastBlock}">
					<li class="page-item">
						<a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=${(curBlock+1)*blockSize + 1}">다음블록</a>
					</li>
				</c:if>
				<c:if test="${pag < totPage}">
					<li class="page-item">
						<a class="page-link text-secondary" href="${ctp}/GuestList.gu?pageSize=${pageSize}&pag=${totPage}">마지막페이지</a>
					</li>
				</c:if>
			</ul>
		</div>
